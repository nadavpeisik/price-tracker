...
- **SQL (Flyway migrations):** SQLFluff, config in `.sqlfluff` at the repo root (issue #228). Same pre-commit hook as ruff (`sqlfluff-fix` + `sqlfluff-lint` on staged files under `db/migration/`); CI lints only migrations **added** on the branch, so each is linted once, in the PR that adds it. V1–V12 predate the config and are `exclude`d from both hooks as **immutable**: `sqlfluff-fix` is an auto-fixer, and Flyway CRCs each line's raw bytes, so an `--all-files` reindent reads as an edit and every DB that already ran them refuses to boot. By hand: `pip install sqlfluff==4.3.0 && sqlfluff lint backend/src/main/resources/db/migration/V13__foo.sql`.
- **`.properties` files must be pure ASCII.** Same pre-commit hook (`ascii-properties` → `scripts/check-ascii-properties.py`, staged `*.properties`). Java `.properties` are spec'd ISO-8859-1 and both readers honor that — Spring Boot's `OriginTrackedPropertiesLoader` decodes ISO-8859-1, IntelliJ defaults the format to it — so a pasted `—` (UTF-8 `E2 80 94`) renders as `â` plus two invisible C1 control characters for every reader. The hook rejects rather than auto-fixes: the ASCII spelling is the author's call. CI re-checks **every** tracked `.properties` file (not just branch-added ones — unlike a migration these stay editable), so `--no-verify` and the web editor don't slip past. By hand: `git ls-files -z '*.properties' | xargs -0 scripts/check-ascii-properties.py`.
...
- **`agy-review.sh`** wraps the Antigravity CLI (`agy -p`), default model **Gemini 3.8 Flash (High)** (`AGY_REVIEW_MODEL`, switch to `Gemini 3.1 Pro (High)` off the free tier).
...
- **Errors: throw an `ApplicationException` subtype (`exception/`), never `ResponseStatusException`** (issue #231). `GlobalExceptionHandler` is the only class that maps a kind of failure to an HTTP status and writes the `ProblemDetail`; a subtype it doesn't know fails loudly as a 500 rather than guessing. Nothing below `controller/` imports `org.springframework.http` or `org.springframework.web.server` (`RestClient` in outbound clients is fine — that's HTTP as a *dependency*, not as our delivery mechanism). `ConflictException` requires an `ErrorCode`, emitted as the `errorCode` member of the ProblemDetail (issue #173): it is what a client switches on, because same-status conflicts have different remedies; `detail` is prose and not a contract. Codes exist only for the 409s — add one when a client needs to distinguish a case, never speculatively.
