...
- **Dashboards:** Grafana 11.4.0 at `localhost:3000` (login from `GRAFANA_ADMIN_USER`/`GRAFANA_ADMIN_PASSWORD` in `.env`; the datasource connects as `grafana_reader`, a SELECT-only Postgres role created by `infra/postgres/init/create-grafana-role.sh` — #242). Provisioned datasource + dashboards under `infra/grafana/`. All time-scoped Postgres panels MUST use the Grafana `$__timeFilter(column)` macro — hardcoded `WHERE x > NOW() - INTERVAL ...` makes the dashboard's time picker inert. New dashboards: drop a JSON into `infra/grafana/dashboards/`; the file provider picks it up every 30s.
